...
- $k_i_min = ceil(k_(i-1)+log_2 log_2 a_(i-1) -log_2 log_2  k_i <=k_i)$

于是可以由$k_(i-1)$推出$k_i$. !

- 注意浮点数的误差

== CF1992D Test for love

#prob[
  一个人在水里游泳(`W`), 总共不能游超过$k$个格子. 水中有鳄鱼(`C`)和木头(`W`). 
  - 从左边(0)移动到右边($L+1$). 有$n$个字符表示第$i$米的河水什么情况
规则:
- 在物品的表面上的时候(陆地/木头): 可以往前跳不超过$m(1<=m<=10)$米;
- 如果在水里的时候, 每次只能往前进1米
- 无论如何不能接触含有鳄鱼的那格子. 
...
#prob[
  定义一个序列的`MAD`为这个序列中最大的出现最多次数的数字. 如果没有数字出现了2次, 值为0. 

  - 给一个大小为$n$的数组$a$, 初始的时候变量`sum=0`
  只要$a$中存在不为0的数, 就重复下列循环: 
  - 令$"sum":="sum"+sum_(i=1)^n a_i$
  - 给定$b$是一个大小为$n$的数组. $b_i:="MAD"([a_1, a_2,...,a_i]), forall i=1..n$, 然后令$a_i := b_i, forall i=1..n$
...
观察2: 只出现1次的数, 操作2次就会消失
- 操作2次后, 其他的数都大于0. 
- 继续右移, 第$i$个数的贡献是$a_i times (n-i+1)$. 
...
- 按照$1,2$的顺序:
  - 第一个人: $(l_0)/r_1$, 第二个人$(l_0l_1)/r_2$
- 按照$2,1$的顺序
  - 第一个人: $(l_0)/r_2$, 第二个人$(l_0l_2)/r_1$

即: 何时$max(l_0/r_1,(l_0l_1)/r_2) < max(l_0/r_2, (l_0l_2)/r_1)$?
  - 化简, 把$l_0$都约去, 有$max(1/r_1, l_1/r_2)<max(1/r_2,l_2/r_1)$?

问: 什么时候交换是有好处的 $->$ 解这样不等式的值

处理的方法: 分类讨论

1. (1)(3)前提是$1/r_1>=l_1/r_2, 1/r_2>=l_2/r_1$, 满足$1/r_1<1/r_2$
  - $(r_2>=l_1r_1) and (r_1>=l_2r_2) and (r_1>r_2).$
  - 不可能的情况!
2. (2)(4) $l_1/r_2>1/r_1, l_2/r_1>1/r_2, l_1/r_2<l_2/r_1$.
  - $(r_2<l_1r_1) and (r_2l_2 > r_1) and (l_1r_1 < l_2r_2)$ $<-$ 排序的依据
3. 4. 请自行推导. 

意味着$l_1r_1<l_2r_2$的要小的会小.

思考题: 在推导的时候, 如果把整数变为$RR$, 就不能贪心了
