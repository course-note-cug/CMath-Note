...
- 如果我们对 $c f[i][j]$ 加上 1 , 从 $a[i][j]$ 到 $a[n][n]$ 都会被加上 1
- 考虑分别对 $c f[i+k][j]$ 和 $c f[i][j+k]$ 都减去 1 。
- 但是从 $a[i+$ $k][j+k]$ 到 $a[n][n]$ 都会被多减去一个 1 , 所以我们需要再把 $c f[i+k][j+k]$ 加 1


== P6070 Decrease

#prob[
	给定一个 $n times n$ 的矩阵, 你可以进行若干次操作。
每次操作, 你可以将一个 $k times k$ 的 连续 子矩阵里的所有数全都加上 1 或者全都减去 1 。
...
思考
- 让$a[i][j]$变为0, 实际上会影响以$(i,j)$起始, 边长为$k$的正方形. 
- $-1$的情况: 如果再也画不出长度为$k$的正方形了
